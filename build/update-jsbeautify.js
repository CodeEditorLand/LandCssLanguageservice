...
update(
	...
	function (contents) {
		...
		if (index1 === -1) {
			throw new Error(
				`Problem patching beautify.css for ESM: '${topLevelFunction}' not found.`,
			);
		}
		if (index2 === -1) {
			throw new Error(
				`Problem patching beautify.css for ESM: '${outputVar}' not found after '${topLevelFunction}'.`,
			);
		}
		if (index3 === -1) {
			throw new Error(
				`Problem patching beautify.css for ESM: '${footer}' not found after '${outputVar}'.`,
			);
		}
		return (
