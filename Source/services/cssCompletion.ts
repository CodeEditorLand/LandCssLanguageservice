...
import { Symbol, Symbols } from "../parser/cssSymbolScope";
...
export class CSSCompletion {
	...
	constructor(
		public variablePrefix: string | null = null,
	...
	public setCompletionParticipants(
		...
		try {
				...
				itemDefaults: {
					editRange: {
						start: {
							line: position.line,
							character:
								position.character - this.currentWord.length,
						},
						end: position,
					},
				},
			...
			for (let i = this.nodePath.length - 1; i >= 0; i--) {
				...
				if (node instanceof nodes.Property) {
					this.getCompletionsForDeclarationProperty(
						node.getParent() as nodes.Declaration,
						result,
					);
					...
					this.getCompletionsForFunctionDeclaration(
						<nodes.FunctionDeclaration>node,
						result,
					);
				} else if (node instanceof nodes.MixinReference) {
					this.getCompletionsForMixinReference(
						<nodes.MixinReference>node,
						result,
					);
					...
					this.getCompletionsForSupportsCondition(
						<nodes.SupportsCondition>node,
						result,
					);
				} else if (node instanceof nodes.ExtendsReference) {
					this.getCompletionsForExtendsReference(
						<nodes.ExtendsReference>node,
						null,
						result,
					);
					...
					node.type === nodes.NodeType.StringLiteral &&
					this.isImportPathParent(node.parent.type)
				) {
	...
	public getCompletionsForDeclarationProperty(
	...
	private getPropertyProposals(
		...
		const triggerPropertyValueCompletion =
			this.isTriggerPropertyValueCompletionEnabled;
		const completePropertyWithSemicolon =
		...
		properties.forEach((entry) => {
			...
			if (declaration && !declaration.semicolonPosition) {
				if (
					...
					this.offset >= this.textDocument.offsetAt(range.end)
				) {
					insertText += "$0;";
				}
			}
			...
			const item: CompletionItem = {
				label: entry.name,
				documentation: languageFacts.getEntryDescription(
					entry,
				...
				for (const restriction of entry.restrictions) {
					switch (restriction) {
						...
						case "timing-function":
							this.getTimingFunctionProposals(
								entry,
								existingNode,
								result,
							);
			...
			for (const existingValue of existingValues.getEntries()) {
				result.items.push({
					...
					textEdit: TextEdit.replace(
						this.getCompletionRange(existingNode),
						existingValue,
					),
	...
	public getCSSWideKeywordProposals(
		...
		for (const func in languageFacts.cssWideFunctions) {
			...
			result.items.push({
				...
				insertTextFormat: SnippetFormat,
				command: strings.startsWith(func, "var")
					? retriggerCommand
					: undefined,
			});
		}
	...
	public getCompletionsForInterpolation(
		node: nodes.Interpolation,
		result: CompletionList,
	): CompletionList {
	...
	public getVariableProposals(
		existingNode: nodes.Node | null,
		result: CompletionList,
	): CompletionList {
		const symbols = this.getSymbolContext().findSymbolsAtOffset(
			this.offset,
			nodes.ReferenceType.Variable,
		);
		for (const symbol of symbols) {
			const insertText = strings.startsWith(symbol.name, "--")
				? `var(${symbol.name})`
				: symbol.name;
			const completionItem: CompletionItem = {
				label: symbol.name,
				documentation: symbol.value
					? strings.getLimitedString(symbol.value)
					: symbol.value,
				textEdit: TextEdit.replace(
					this.getCompletionRange(existingNode),
					insertText,
			...
			if (
				...
				languageFacts.isColorString(completionItem.documentation)
			) {
				completionItem.kind = CompletionItemKind.Color;
			}

			if (symbol.node.type === nodes.NodeType.FunctionParameter) {
				const mixinNode = <nodes.MixinDeclaration>(
	...
	public getVariableProposalsForCSSVarFunction(
		result: CompletionList,
	): CompletionList {
		...
		let symbols = this.getSymbolContext().findSymbolsAtOffset(
			this.offset,
			nodes.ReferenceType.Variable,
		);
		for (const symbol of symbols) {
	...
	public getUnitProposals(
		...
		if (
			existingNode &&
			existingNode.parent &&
			existingNode.parent.type === nodes.NodeType.Term
		) {
	...
	protected getCompletionRange(existingNode: nodes.Node | null) {
		if (
			existingNode &&
			existingNode.offset <= this.offset &&
			this.offset <= existingNode.end
		) {
			const end =
				existingNode.end !== -1
					? this.textDocument.positionAt(existingNode.end)
					: this.position;
	...
	public getCompletionsForRuleSet(
		...
		const isAfter =
			declarations &&
			declarations.endsWith("}") &&
			this.offset >= declarations.end;
		if (isAfter) {
			return this.getCompletionForTopLevel(result);
		}
		const isInSelectors =
		...
		if (isInSelectors) {
			return this.getCompletionsForSelector(
				ruleSet,
				ruleSet.isNested(),
				result,
			);
		...
		return this.getCompletionsForDeclarations(
			ruleSet.getDeclarations(),
			result,
		);
	}
	...
	public getCompletionsForSelector(
		...
		const existingNode = this.findInNodePath(
		...
		pseudoClasses.forEach((entry) => {
			...
			const item: CompletionItem = {
				...
				kind: CompletionItemKind.Function,
				insertTextFormat:
					entry.name !== insertText ? SnippetFormat : void 0,
			};
		...
		if (ruleSet && ruleSet.isNested()) {
			const selector = ruleSet
				.getSelectors()
				.findFirstChildBeforeOffset(this.offset);
			if (
				selector &&
				ruleSet.getSelectors().getChildren().indexOf(selector) === 0
			) {
	...
	public getCompletionsForDeclarations(
		...
		if (node instanceof nodes.AbstractDeclaration) {
			...
			if (
				!isDefined(declaration.colonPosition) ||
				this.offset <= declaration.colonPosition
			) {
				// complete property
				return this.getCompletionsForDeclarationProperty(
					declaration as nodes.Declaration,
					result,
				);
			} else if (
				isDefined(declaration.semicolonPosition) &&
				declaration.semicolonPosition < this.offset
			) {
			...
			if (declaration instanceof nodes.Declaration) {
				// complete value
				return this.getCompletionsForDeclarationValue(
					declaration,
					result,
				);
			}
	...
	public getCompletionsForExpression(
		...
		if (parent instanceof nodes.FunctionArgument) {
			this.getCompletionsForFunctionArgument(
				parent,
				<nodes.Function>parent.getParent(),
				result,
			);
			return result;
		}
		...
		const declaration = <nodes.Declaration>(
			expression.findParent(nodes.NodeType.Declaration)
		);
		if (!declaration) {
	...
	public getCompletionsForFunctionArgument(
		...
		if (identifier && identifier.matches("var")) {
			if (
				!func.getArguments().hasChildren() ||
				func.getArguments().getChild(0) === arg
			) {
	...
	public getCompletionsForFunctionDeclaration(
		decl: nodes.FunctionDeclaration,
		result: CompletionList,
	): CompletionList {
		const declarations = decl.getDeclarations();
		if (
			declarations &&
			this.offset > declarations.offset &&
			this.offset < declarations.end
		) {
			this.getTermProposals(undefined, null, result);
		}
	...
	public getCompletionsForMixinReference(
		ref: nodes.MixinReference,
		result: CompletionList,
	): CompletionList {
		const allMixins = this.getSymbolContext().findSymbolsAtOffset(
			this.offset,
			nodes.ReferenceType.Mixin,
		);
		for (const mixinSymbol of allMixins) {
			if (mixinSymbol.node instanceof nodes.MixinDeclaration) {
				result.items.push(
					this.makeTermProposal(
						mixinSymbol,
						mixinSymbol.node.getParameters(),
						null,
					),
				);
			}
	...
	public getTermProposals(
		...
		const allFunctions = this.getSymbolContext().findSymbolsAtOffset(
			this.offset,
			nodes.ReferenceType.Function,
		);
		for (const functionSymbol of allFunctions) {
			if (functionSymbol.node instanceof nodes.FunctionDeclaration) {
				result.items.push(
					this.makeTermProposal(
						functionSymbol,
						functionSymbol.node.getParameters(),
						existingNode,
					),
				);
			}
	...
	private makeTermProposal(
		...
		const params = parameters.getChildren().map((c) => {
			return c instanceof nodes.FunctionParameter
				? (<nodes.FunctionParameter>c).getName()
				: c.getText();
		});
		const insertText =
			symbol.name +
	...
	public getCompletionsForSupportsCondition(
		...
		if (child) {
			if (child instanceof nodes.Declaration) {
				if (
					!isDefined(child.colonPosition) ||
					this.offset <= child.colonPosition
				) {
					return this.getCompletionsForDeclarationProperty(
						child,
						result,
		...
		if (
			...
			(!isDefined(supportsCondition.rParent) ||
				this.offset <= supportsCondition.rParent)
		) {
			return this.getCompletionsForDeclarationProperty(null, result);
		}
		return result;
	...
	public getCompletionsForSupports(
		supports: nodes.Supports,
		result: CompletionList,
	): CompletionList {
	...
	public getCompletionForUriLiteralValue(
		uriLiteralNode: nodes.Node,
		result: CompletionList,
	): CompletionList {
		let uriValue: string;
	...
	public getCompletionForImportPath(
		importPathNode: nodes.Node,
		result: CompletionList,
	): CompletionList {
		this.completionParticipants.forEach((participant) => {
	...
	private hasCharacterAtPosition(offset: number, char: string) {
		...
		return (
			offset >= 0 && offset < text.length && text.charAt(offset) === char
		);
	}
...
function isDeprecated(entry: languageFacts.IEntry2): boolean {
	if (
		entry.status &&
		(entry.status === "nonstandard" || entry.status === "obsolete")
	) {
		return true;
	}
	...
	public visitNode(node: nodes.Node): boolean {
		if (
			...
			(node instanceof nodes.Function &&
				languageFacts.isColorConstructor(<nodes.Function>node))
		) {
			if (
				this.currentOffset < node.offset ||
				node.end < this.currentOffset
			) {
