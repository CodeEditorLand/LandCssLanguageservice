...
import { CSSIssueType, ParseError } from "./cssErrors";
...
export class Parser {
	...
	public acceptOneKeyword(keywords: string[]): boolean {
		if (TokenType.AtKeyword === this.token.type) {
			for (const keyword of keywords) {
				if (
					keyword.length === this.token.text.length &&
	...
	public resync(
		resyncTokens: TokenType[] | undefined,
		resyncStopTokens: TokenType[] | undefined,
	): boolean {
		while (true) {
			if (resyncTokens && resyncTokens.indexOf(this.token.type) !== -1) {
				...
				resyncStopTokens &&
				resyncStopTokens.indexOf(this.token.type) !== -1
			) {
				return true;
			} else {
	...
	public internalParse<T extends nodes.Node, U extends T | null>(
		input: string,
		parseFunc: () => U,
		textProvider?: nodes.ITextProvider,
	): U;

	public internalParse<T extends nodes.Node, U extends T>(
		input: string,
		parseFunc: () => U,
		textProvider?: nodes.ITextProvider,
	): U {
	...
	public _parseStylesheetAtStatement(
		isNested: boolean = false,
	): nodes.Node | null {
		return (
			this._parseImport() ||
	...
	public _parseDeclarations(
		...
		while (node.addChild(decl)) {
			...
			if (
				this._needsSemicolonAfter(decl) &&
				!this.accept(TokenType.SemiColon)
			) {
			...
			if (
				decl &&
				this.prevToken &&
				this.prevToken.type === TokenType.SemiColon
			) {
				(decl as nodes.Declaration).semicolonPosition =
					this.prevToken.offset;
			}
	...
	public _parseBody<T extends nodes.BodyDeclaration>(
		node: T,
		parseDeclaration: () => nodes.Node | null,
	): T {
	...
	public _tryParseCustomPropertyDeclaration(
		stopTokens?: TokenType[],
	): nodes.CustomPropertyDeclaration | null {
	 ...
	 * of delimitors).
	 */
	public _parseCustomPropertyValue(
	...
	public _parseNamespace(): nodes.Node | null {
		...
		if (!node.addChild(this._parseURILiteral())) {
			...
			if (
				...
				return this.finish(node, ParseError.URIExpected, [
					TokenType.SemiColon,
				]);
			}
		}
	...
	public _parsePropertyAtRule(): nodes.Node | null {
		...
		if (
			!this.peekRegExp(TokenType.Ident, /^--/) ||
			!node.setName(this._parseIdent([nodes.ReferenceType.Property]))
		) {
			return this.finish(node, ParseError.IdentifierExpected);
		}
	...
	public _parseLayer(isNested: boolean = false): nodes.Node | null {
		...
		if (
			(!names || names.getChildren().length === 1) &&
			this.peek(TokenType.CurlyL)
		) {
			return this._parseBody(
				node,
				this._parseLayerDeclaration.bind(this, isNested),
			);
		}
	...
	public _parseSupports(isNested = false): nodes.Node | null {
		...
		return this._parseBody(
			node,
			this._parseSupportsDeclaration.bind(this, isNested),
		);
	}
	...
	private _parseSupportsConditionInParens(): nodes.Node {
		...
		if (this.accept(TokenType.ParenthesisL)) {
			...
			if (
				!node.addChild(
					this._tryToParseDeclaration([TokenType.ParenthesisR]),
				)
			) {
	...
	public _parseMedia(isNested = false): nodes.Node | null {
		...
		return this._parseBody(
			node,
			this._parseMediaDeclaration.bind(this, isNested),
		);
	}
	...
	public _parseContainer(isNested: boolean = false): nodes.Node | null {
		...
		return this._parseBody(
			node,
			this._parseContainerDeclaration.bind(this, isNested),
		);
	}
	...
	public _parseSimpleSelector(): nodes.SimpleSelector | null {
		...
		while (
			(c === 0 || !this.hasWhitespace()) &&
			node.addChild(this._parseSimpleSelectorBody())
		) {
			c++;
		}
		...
		if (node) {
			if (!this.hasWhitespace() && this.accept(TokenType.ParenthesisL)) {
				const tryAsSelector = () => {
					...
					while (
						this.accept(TokenType.Comma) &&
						selectors.addChild(this._parseSelector(true))
					) {
						// loop
					}
	...
	public _parseFunction(): nodes.Function | null {
		...
		if (node.getArguments().addChild(this._parseFunctionArgument())) {
			while (this.accept(TokenType.Comma)) {
				...
				if (
					!node.getArguments().addChild(this._parseFunctionArgument())
				) {
