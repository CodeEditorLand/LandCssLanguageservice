...
export class LESSParser extends cssParser.Parser {
	...
	public _parseImport(): nodes.Node | null {
		if (
			!this.peekKeyword("@import") &&
			!this.peekKeyword("@import-once") /* deprecated in less 1.4.1 */
		) {
			return null;
		}
	...
	public _parseDetachedRuleSet(): nodes.Node | null {
		...
		if (this.peekDelim("#") || this.peekDelim(".")) {
			...
			if (!this.hasWhitespace() && this.accept(TokenType.ParenthesisL)) {
				let node = <nodes.MixinDeclaration>(
				...
				if (
					...
					while (
						...
						if (
							!node
								.getParameters()
								.addChild(this._parseMixinParameter())
						) {
							this.markError(
								node,
	...
	public _parseLookupValue(): nodes.Node | null {
		...
		if (
			((node.addChild(this._parseVariable(false, true)) ||
	...
	public _parseVariable(
		declaration: boolean = false,
		insideLookup: boolean = false,
	): nodes.Variable | null {
	...
	public _parseEscaped(): nodes.Node | null {
		if (
			this.peek(TokenType.EscapedJavaScript) ||
		...
		if (this.peekDelim("~")) {
			...
			if (
				...
				this.accept(TokenType.EscapedJavaScript)
			) {
				return this.finish(node);
			} else {
	...
	public _parseKeyframeIdent(): nodes.Node | null {
		return (
			this._parseIdent([nodes.ReferenceType.Keyframe]) ||
			this._parseVariable()
		);
	}
	...
	public _parseSelector(isNested: boolean): nodes.Selector | null {
		...
		while (node.addChild(this._parseSimpleSelector())) {
			...
			if (
				node.addChild(this._parseGuard()) &&
	...
	public _parsePropertyIdentifier(
		inLookup: boolean = false,
	): nodes.Identifier | null {
		const propertyRegex = /^[\w-]+/;

		if (
			!this.peekInterpolatedIdent() &&
			!this.peekRegExp(this.token.type, propertyRegex)
		) {
			return null;
		}
	...
	public _acceptInterpolatedIdent(
		node: nodes.Node,
		identRegex?: RegExp,
	): boolean {
		let hasContent = false;
		...
		while (
			accept() ||
			node.addChild(
				this._parseInterpolation() || this.try(indentInterpolation),
			)
		) {
			hasContent = true;
	...
	public _tryParseMixinDeclaration(): nodes.Node | null {
		...
		if (
			!node.setIdentifier(this._parseMixinDeclarationIdentifier()) ||
			!this.accept(TokenType.ParenthesisL)
		) {
			this.restoreAtMark(mark);
	...
	private _parseMixinDeclarationIdentifier(): nodes.Identifier | null {
		...
		if (this.peekDelim("#") || this.peekDelim(".")) {
			...
			if (
				this.hasWhitespace() ||
				!identifier.addChild(this._parseIdent())
			) {
				return null;
			}
	...
	public _parseDetachedRuleSetMixin(): nodes.Node | null {
		...
		if (
			node.addChild(this._parseVariable(true)) &&
			(this.hasWhitespace() || !this.accept(TokenType.ParenthesisL))
		) {
			this.restoreAtMark(mark);
			...
			if (node.getArguments().addChild(this._parseMixinArgument())) {
				while (
					...
					if (
						!node
							.getArguments()
							.addChild(this._parseMixinArgument())
					) {
	...
	public _parseMixinArgument(): nodes.Node | null {
		...
		if (
			node.setValue(this._parseDetachedRuleSet() || this._parseExpr(true))
		) {
			return this.finish(node);
		}
