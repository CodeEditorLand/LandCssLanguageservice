...
export class SCSSParser extends cssParser.Parser {
	...
	public _parseStylesheetStatement(
	...
	public _parseVariableDeclaration(
		panic: TokenType[] = [],
	): nodes.VariableDeclaration | null {
		if (!this.peek(scssScanner.VariableName)) {
			return null;
		}

		const node = <nodes.VariableDeclaration>(
			this.create(nodes.VariableDeclaration)
		);
	...
	public _parseMediaFeatureRangeOperator(): boolean {
		return (
			this.accept(scssScanner.SmallerEqualsOperator) ||
			this.accept(scssScanner.GreaterEqualsOperator) ||
	...
	): nodes.Identifier | null {
		if (
			!this.peek(TokenType.Ident) &&
			!this.peek(scssScanner.InterpolationFunction) &&
			!this.peekDelim("-")
		) {
			return null;
		}
	...
	public _parseDeclaration(
		...
		if (!this.accept(TokenType.Colon)) {
			return this.finish(
				...
				[TokenType.Colon],
				stopTokens || [TokenType.SemiColon],
			);
		}
		if (this.prevToken) {
			node.colonPosition = this.prevToken.offset;
	...
	public _parseSimpleSelectorBody(): nodes.Node | null {
		return (
			this._parseSelectorPlaceholder() || super._parseSimpleSelectorBody()
		);
	}
	...
	public _parseElementName(): nodes.Node | null {
		...
		if (
			node &&
			!this.hasWhitespace() &&
			this.peek(TokenType.ParenthesisL)
		) {
			// for #49589
			this.restoreAtMark(pos);
			return null;
	...
	public _parseControlStatement(
		parseStatement: () => nodes.Node | null = this._parseRuleSetDeclaration.bind(
			this,
		),
	): nodes.Node | null {
		if (!this.peek(TokenType.AtKeyword)) {
	...
	private _internalParseIfStatement(
		...
		if (this.acceptKeyword("@else")) {
			if (this.peekIdent("if")) {
				node.setElseClause(
					this._internalParseIfStatement(parseStatement),
				);
			} else if (this.peek(TokenType.CurlyL)) {
				const elseNode = <nodes.BodyDeclaration>(
					this.create(nodes.ElseStatement)
				);
	...
	public _parseForStatement(
		...
		if (!this.acceptIdent("from")) {
			return this.finish(node, SCSSParseError.FromExpected, [
				TokenType.CurlyR,
			]);
		}
		...
		if (!this.acceptIdent("to") && !this.acceptIdent("through")) {
			return this.finish(node, SCSSParseError.ThroughOrToExpected, [
				TokenType.CurlyR,
			]);
		}
		if (!node.addChild(this._parseBinaryExpr())) {
	...
	public _parseFunctionDeclaration(): nodes.Node | null {
		...
		const node = <nodes.FunctionDeclaration>(
			this.create(nodes.FunctionDeclaration)
		);
		this.consumeToken(); // @function

		if (
			!node.setIdentifier(
				this._parseIdent([nodes.ReferenceType.Function]),
			)
		) {
			return this.finish(node, ParseError.IdentifierExpected, [
				TokenType.CurlyR,
		...
		if (node.getParameters().addChild(this._parseParameterDeclaration())) {
			while (this.accept(TokenType.Comma)) {
				...
				if (
					!node
						.getParameters()
						.addChild(this._parseParameterDeclaration())
				) {
					return this.finish(node, ParseError.VariableNameExpected);
				}
			}
	...
	public _parseMixinDeclaration(): nodes.Node | null {
		...
		const node = <nodes.MixinDeclaration>(
			this.create(nodes.MixinDeclaration)
		);
		this.consumeToken();
		...
		if (this.accept(TokenType.ParenthesisL)) {
			if (
				node.getParameters().addChild(this._parseParameterDeclaration())
			) {
				while (this.accept(TokenType.Comma)) {
					if (this.peek(TokenType.ParenthesisR)) {
						break;
	...
	public _parseParameterDeclaration(): nodes.Node | null {
		const node = <nodes.FunctionParameter>(
			this.create(nodes.FunctionParameter)
		);
			...
			if (node.getArguments().addChild(this._parseFunctionArgument())) {
				while (this.accept(TokenType.Comma)) {
					...
					if (
						!node
							.getArguments()
							.addChild(this._parseFunctionArgument())
					) {
						return this.finish(node, ParseError.ExpressionExpected);
					}
				}
	...
	public _parseFunctionArgument(): nodes.Node | null {
		...
		const node = <nodes.FunctionArgument>(
			this.create(nodes.FunctionArgument)
		);
	...
	public _parseUse(): nodes.Node | null {
		...
		if (!this.peek(TokenType.SemiColon) && !this.peek(TokenType.EOF)) {
			...
			if (
				this.acceptIdent("as") &&
				!node.setIdentifier(
					this._parseIdent([nodes.ReferenceType.Module]),
				) &&
				!this.acceptDelim("*")
			) {
				return this.finish(
					node,
					ParseError.IdentifierOrWildcardExpected,
				);
			}
	...
	public _parseModuleConfigDeclaration(): nodes.Node | null {
		const node = <nodes.ModuleConfiguration>(
			this.create(nodes.ModuleConfiguration)
		);
		...
		if (
			...
			!node.setValue(this._parseExpr(true))
		) {
			return this.finish(
				node,
				ParseError.VariableValueExpected,
	...
	public _parseForwardVisibility(): nodes.Node | null {
		const node = <nodes.ForwardVisibility>(
			this.create(nodes.ForwardVisibility)
		);
